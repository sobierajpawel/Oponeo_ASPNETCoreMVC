...
namespace Oponeo.CustomerManagementMVC.WebApp.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public async Task<IActionResult> Search(string searchString, int pageNumber)
        {
            ...
            if (!_memoryCache.TryGetValue(memoryKey, out IList<ProductViewModel> productsViewModels))
            {
                ...
                productsViewModels = productsViewModels.Skip(pageNumber * PAGE_SIZE).Take(PAGE_SIZE).ToList();

                var memoryCacheOption = new MemoryCacheEntryOptions().SetSlidingExpiration(TimeSpan.FromSeconds(30));

                _memoryCache.Set(memoryKey, productsViewModels, memoryCacheOption);
            }
