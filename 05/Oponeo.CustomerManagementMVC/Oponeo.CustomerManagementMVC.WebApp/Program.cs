...
{
    options.AddPolicy("RequiredCreatorRole", policy =>
        policy.RequireRole("creator")
        .RequireClaim("allowed_modules","products")
        .AddRequirements(new MinimumAgeRequirement(5)));

    options.AddPolicy("RequireModuleProducts", policy =>
        policy.RequireClaim("allowed_modules", "products"));
});
