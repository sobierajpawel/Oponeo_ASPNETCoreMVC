...
namespace Oponeo.CustomerManagementMVC.WebApp.Controllers
{
    public class ProductsController : Controller
    {
        ...
        {
            if (!_memoryCache.TryGetValue(searchViewModel.SearchString, out IList<ProductViewModel> productsViewModels))
            {
                productsViewModels = new List<ProductViewModel>();
                await Task.Delay(2000);
                foreach (var product in _productService.FindByFilter(searchViewModel.SearchString))
                {
                ...
                _memoryCache.Set(searchViewModel.SearchString, productsViewModels, memoryCacheOption);
            }

            return PartialView("GetProducts", productsViewModels);
        }
